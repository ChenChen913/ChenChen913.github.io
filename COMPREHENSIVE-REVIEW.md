...
| ⏸️ P3 | 核对"外部程序设计"课程名（F8） ⏸️ 需核对成绩单确认（内容项，待所有者核实） | 本报告 |
| ✅ P3 | 自定义 404 页面（F9） ✅ 已修复（2026-07-29，commit e7aaca5） | 本报告 |
| ⏸️ P3 | 横版 og:image（F10） ⏸️ 需 1200×630 横版图素材（待所有者提供） | 本报告 |
| ✅ P3 | CI 加 htmlproofer/文档引用校验（F11） ✅ 已修复（2026-07-29 e7aaca5；2026-07-30 f059869 修复 5.x 不兼容参数，实测 18 内部链接 0 死链） | 双方 |
| ✅ P3 | 路径写法统一为 relative_url（F13） ✅ 已修复（2026-07-29，commit e7aaca5） | 本报告 |
| ✅ P3 | PDF 查看器渲染任务取消（F15） ✅ 已修复（2026-07-29，commit e7aaca5） | 本报告 |
| ✅ P3 | skip-link（F16） ✅ 已修复（2026-07-29，commit e7aaca5；htmlproofer 锚点校验通过） | 双方 |
| ✅ P3 | 文件末尾杂项清理（F17） ✅ 已修复（2026-07-29，commit e7aaca5） | 本报告 |
| ⏸️ P3 | 内联样式归并 / ES6 现代化（F14 + 其 F-P3-3/4/5） ✅ 空监听器已清理（2026-07-28 P2 修复时）；内联样式归并与 ES6 现代化暂缓（改动量大、风险高，另择时机） | INDEPENDENT-AUDIT |
...
两份报告的关系：INDEPENDENT-AUDIT.md 是一次**大规模的存量问题清剿**（37 项，修复率 34/37），它完成后项目质量发生了跃迁——这正是我第 2 部分给出 A− 而它当年只给 3.4/5.0 的原因：**我们审的实际上是同一个项目的两个时代**。我的报告价值在于：① 验证了它的修复真实落地、状态无虚报；② 找出了它的 3 处内部矛盾和 1 处修复引入的新问题（F2）；③ 在它清剿后的"干净地面"上又找出 10 项它未覆盖的新问题。二者融合后的有效待办即 5.4 节清单，P1 两项、P2 五项（F3—F7）均已修复；P3 六项（F9/F11/F13/F15/F16/F17）亦已修复并经 CI 验证（htmlproofer 实测 18 内部链接 0 死链）。截至 2026-07-30，技术性待办仅余 F8（课程名核对，需成绩单）、F10（og:image，需横版图素材）、F14/ES6 现代化（改动量大另择时机）三项，以及所有者已知悉暂缓的内容项。
