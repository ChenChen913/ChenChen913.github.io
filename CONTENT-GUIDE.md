...
>
> 链接必须是 `http://` 或 `https://` 开头的完整 URL；模板只放行这两种协议，其余协议（如 `javascript:`）会输出为 `#`。
...
>
> `pdf` 字段只填 `assets/publications/` 下的裸文件名（如 `foo.pdf`），不要包含 `/`、引号等特殊字符；模板会转义属性值并拒绝 `..` 路径。
