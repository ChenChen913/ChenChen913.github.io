...
> ⚠️ **关键一步：锁定 Linux 平台**（Vercel 和 Cloudflare Pages 构建环境使用 Linux x86_64）。如果本地开发用 macOS 或 Windows，**必须**运行以下命令并提交更新后的 `Gemfile.lock`：
> ```bash
> bundle lock --add-platform x86_64-linux
> ```
> 不执行这一步，Vercel 和 Cloudflare Pages 部署时可能报 `Gem::Ext::BuildError` 或 nokogiri 等原生扩展编译失败。**GitHub Actions 和 Netlify 同样建议加上，确保跨平台兼容。**
...
> ⚠️ **Vercel 最容易踩的坑：** 构建环境使用 Linux x86_64，本地如果是 macOS/Windows 必须先执行 `bundle lock --add-platform x86_64-linux` 并提交更新的 `Gemfile.lock`，否则原生扩展（如 nokogiri）编译失败。
